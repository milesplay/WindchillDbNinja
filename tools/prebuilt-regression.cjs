...
async function fixture(t) {
  ...
  const target = {os: 'win32', arch: 'x64', javaMajor: 17, windchill: '13.0.2.6', windchillBuild: 33,
...
test('prebuilt package requires exact module inventory, current sources and matching generated model artifacts', async t => {
  ...
  assert.equal(result.manifest.target.windchill, '13.0.2.6');
  assert.equal(result.manifest.target.windchillBuild, 33);
...
test('target-CCD candidate manifests require annotation-processed provenance', async t => {
  const {readPrebuilt} = await api;
  const f = await fixture(t);
  f.manifest.build.method = 'target-ccd';
  f.manifest.build.annotationProcessing = true;
  f.save();
  assert.equal(readPrebuilt(f.directory).manifest.build.method, 'target-ccd');
  f.manifest.build.annotationProcessing = false;
  f.save();
  assert.throws(() => readPrebuilt(f.directory), /Unsupported or incomplete prebuilt manifest/);
});
...
test('the qualified prebuilt profile cannot claim Linux, another Java major or non-Oracle support', async t => {
  const {readPrebuilt} = await api;
  for (const [key, value] of [['os', 'linux'], ['arch', 'arm64'], ['javaMajor', 11], ['oracleMajor', 23]]) {
    const f = await fixture(t);
...
test('target verification requires exact JDK, SDK hashes and Windchill support datecode', {
  ...
  const probe = (major, release, build = 33) => {
    const family = release.split('.').slice(0, 2).join('.');
