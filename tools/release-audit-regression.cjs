...
test('publication rejects a link within an included tree', t => {
  const {directory, source} = publicationFixture(t);
  const outside = path.join(directory, 'outside-linked');
  fs.mkdirSync(outside);
  fs.symlinkSync(outside, path.join(source, 'tools/linked'), process.platform === 'win32' ? 'junction' : 'dir');
  const result = publication(source);
  assert.notEqual(result.status, 0);
  assert.match(result.stderr, /Do not publish symlinks/);
});

test('publication rejects a linked customization ancestor', t => {
  const {directory, source} = publicationFixture(t);
  const outside = path.join(directory, 'outside-customization');
  fs.renameSync(path.join(source, 'customization'), outside);
  fs.symlinkSync(outside, path.join(source, 'customization'), process.platform === 'win32' ? 'junction' : 'dir');
  const result = publication(source);
...
test('deployment confinement rejects a dangling link before any write', async t => {
  const directory = fixture(t);
  const home = path.join(directory, 'mock-home');
  fs.mkdirSync(home);
  const outside = path.join(directory, 'outside-link');
  fs.mkdirSync(outside);
  fs.symlinkSync(outside, path.join(home, 'dangling'), process.platform === 'win32' ? 'junction' : 'dir');
  fs.rmSync(outside, {recursive: true});
  const {confined} = await import('./dbninja.mjs');
  assert.throws(() => confined(home, 'dangling'), /symbolic/);
});
...
function rollbackFixture(t) {
  ...
  const xconfRelative = process.platform === 'win32' ? 'bin/xconfmanager.bat' : 'bin/xconfmanager';
  const xconf = put(home, xconfRelative, process.platform === 'win32'
  ...
  const plan = {format: 1, target: fs.realpathSync(home), javascriptOnly: true,
    ...
    toolchain: {[xconfRelative]: hash(fs.readFileSync(xconf))}};
...
test('mock rollback restores an unchanged snapshot using only a fake xconf validator', t => {
...
test('mock rollback rejects incomplete, changed-live and corrupted-backup snapshots before restoring', t => {
  for (const problem of ['incomplete', 'live-change', 'backup-change', 'toolchain-change']) {
    ...
    else fs.appendFileSync(path.join(f.home, process.platform === 'win32'
      ? 'bin/xconfmanager.bat' : 'bin/xconfmanager'), '# changed toolchain\n');
    const result = f.run();
...
test('mock JavaScript-only rollback refuses changed non-JavaScript prerequisites', t => {
  const f = rollbackFixture(t);
