...
public final class SqlEvidenceCapture implements AutoCloseable {
   ...
   private void installLocked() throws IOException {
      ...
      synchronized (configuration) {
         if (context.getConfiguration() != configuration || configuration.getFilter() != null) {
      ...
      if (context.getConfiguration() != configuration || configuration.getFilter() != filter
   ...
   private void tick() {
      try {
         ...
         } else if (context.getConfiguration() != configuration || configuration.getFilter() != filter
   ...
   private Filter.Result observe(Logger logger, Level level, Object message) {
      ...
      try {
         ...
         for (StackWalker.StackFrame frame : frames) {
            if (frame.getClassName().startsWith("com.ptc.dbcapture.")
                  && !frame.getClassName().startsWith("com.ptc.dbcapture.diagnostics.")) {
               synchronized (dataLock) { counters[2]++; }
               return original;
            }
         }
